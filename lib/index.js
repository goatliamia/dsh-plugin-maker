// dsh-plugin-maker host half: scaffold（生成）+ check（契约校验/发布合规/升级基线/密钥自查）+ vet/adopt（接盘改造）+ checklist（动作清单）+ impact（引用面扫描）+ surface（工具面诊断）
...
import { clientEntryOf, dshVersion, checkPlugin, scaffoldFiles, migrationHits, hookSuggestions, vetPlugin } from './check-core.mjs'
import { analyzePlugin, renderProposal, renderReport } from './surface-analyze.mjs'
...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: '动作清单（硬约束的执行入口）：输入任务类型（开工/改文件改代码/发版本/废文档改名语义变更/调查技术可行性），返回该任务必须执行的动作清单。开工或接手任何任务前先调用本工具拿清单，照单执行，不靠读完文档即过。',
    parameters: {
      taskType: { type: 'string', required: true, description: '任务类型关键词，如 发版本 / 改文件 / 废文档 / 改名 / 调查 / 开工' },
    },
  ...
  // 工具面诊断：这个插件注册了多少 model-facing 工具、哪些像实现原语、值不值得收窄。
  // 只读；判据是"有没有稳定组合"，不是"工具多不多"——结论可以是不建议做。
  ctx.tools.register(defineTool({
    name: 'plugin_maker_surface',
    description: '工具面诊断（只读）：扫描一个插件目录，列出它注册的 model-facing 工具、按前缀分组、标出像实现原语的那些，并给出"是否值得收窄成语义操作"的判断与起步声明。判据是插件内部有没有稳定组合，不是工具数量——结论可以是不建议做 facade。只读，不修改任何文件。',
    parameters: {
      targetDir: { type: 'string', required: true, description: '插件目录绝对路径' },
      propose: { type: 'boolean', description: 'true 时附上候选操作的起步声明（默认 false）' },
    },
    output: { schema: { type: 'string' }, render(_a, v) { return [{ type: 'text', text: v }] } },
    presentCall(args) { return { card: 'generic', title: '工具面诊断', rawInput: String(args.targetDir ?? '') } },
    presentResult(args, result) {
      if (result.isError) return undefined
      return { card: 'generic', title: '工具面诊断：' + path.basename(String(args.targetDir ?? '')), content: result.content }
    },
    async execute(args) {
      const dir = String(args.targetDir ?? '').trim()
      if (!path.isAbsolute(dir)) return 'targetDir 必须是绝对路径'
      const report = await analyzePlugin(dir)
      let text = renderReport(report)
      if (args.propose === true && report.candidates.length > 0) {
        text += '\n\n---\n'
        for (const candidate of report.candidates) {
          text += '\n## 起步声明（`' + candidate.group + '`）\n\n```js\n' + renderProposal(candidate) + '\n```\n'
        }
      }
      return text
    },
