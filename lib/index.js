...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const rows = [
        ...
        ok(/export\s+function\s+apply/.test(hostAll)) + ' host ESM export apply',
        ...
        ok(!/required:\s*false\s*[,}]/.test(all)) + ' 无 required:false',
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (!/export\s+function\s+apply/.test(hostAll)) fix('host 没有 export function apply', 'host 入口（' + hostEntry + '）补 apply(ctx)，或在入口 export * 转发到真正含 apply 的模块')
