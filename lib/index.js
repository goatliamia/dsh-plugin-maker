...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      const rows = [
        ...
        ...( /inject\s*(?:=|:|[(])\s*\[[^\]]*apiProxy/.test(hostAll) ? ['⚠️ apiProxy 0.1.2 迁移事实：该服务在 0.1.2 已整体移除（packages/host/apiproxy）——0.1.1 可用；升级前先迁移：宿主平面直连领域服务（llm 等），客户端平面走 ctx.remote.* 并声明 dsh.client（实测：oh-my-dsh Docker 全链路复现，官方 #5120 讨论）'] : [] ),
        '密钥自查（⑧）:',
        ...secretScan(dir),
        '危险操作自查（自我指涉）:',
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args) {
      ...
      if (/inject\s*(?:=|:|[(])\s*\[[^\]]*apiProxy/.test(hostAll)) fix('注入 apiProxy（0.1.2 已整体移除，入口将 pending 拖死宿主启动）', '升级到 0.1.2 前必须迁移——宿主平面：跳过网关门面、直连领域服务（llm/sessionTitle 等）；客户端平面：走 ctx.remote.* 并在 package.json 声明 dsh.client。注意：别把 apiProxy 换成 remote（那是客户端平面门面，宿主插件注入它仍会 pending）。对照 ALPHA1-01 映射表（oh-my-dsh/dsh-plugin-upgrade-skill，Docker 实测验证）')
