...
// 域划分（2026-09-09 收窄）：maker 只做开发期机械面。错误域/踩坑沉淀/模式表归 dsh-retro，
// 协作协议层（收件箱/change 信号/Decision）归 project-context-bridge——maker 不再复制它们的条目。
// 跨域条目标注〔归口〕：未装对应插件时跳过该条，不影响 maker 工具本身可用（详见 docs/standalone.md）。
export const CHECKLISTS = {
  ...
  '发版本': [
    ...
    '重要 bug 修复同一 commit 带回归测试（机器可验证：测试全绿）；**不写教训/踩坑档案**——那类沉淀会随版本过期变有害，也不属于 maker 的域',
    'plugin_maker_check 全绿（契约/发布合规/密钥/waterfall 透传）',
    '更新桥 ExternalRef（版本/仓库身份）〔归口：桥，未装则跳过〕',
    '**更新 host 常驻插件后，主动提醒用户重启 DSH 才生效**；纯工具类插件（maker 工具）新会话生效——发版完即说，别等用户问',
...
export function checklistFor(taskType) {
  ...
  if (key) return { type: key, items: CHECKLISTS[key] }
  // 无匹配 → 开工清单。踩坑/沉淀类输入不再有专属清单：那属于 dsh-retro 的域，maker 不复制。
  return { type: '开工', items: CHECKLISTS['开工'] }
}

/** 协作条目过滤（去我们化·不误伤）：hasBridge 为假时隐藏标注桥归口的条目。
 *  检测源=协作工具是否已注册（project_inbox=桥侧入口）；
 *  未装协作件的对外环境自动得到干净清单，装了的全量可见。 */
export function visibleItems(items, { hasBridge = true } = {}) {
