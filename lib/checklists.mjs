...
export const CHECKLISTS = {
  ...
  '调查/技术可行性': [
    '**先查底座**：官方/已有机制已正确覆盖需求 → 出口为 Reuse / 不做；「无需新增机制」是有效调查结论（判据=真正满足需求：不是"看起来像/理论上可以/社区有人做过"，且**能接进我们的机制链（真源/信号/状态/语义）**——轮子存在但接不进 → 重合部分复用、接不上的缺口最小自建，不因"有轮子"就不做）',
    ...
    '**吸收外部知识：先分信息性质，再定落点**（知识吸收定案）：三层性质（事实性/技术性/调性）只是帮理解它是什么；真正决定落点的是「是否值得系统承担」——值得承担 → 检查/工具/工作流，不值得承担 → 文档/参考。机制化六条判据：①明确事实依据 ②能实测 ③能感知上游变化（check 规则须与 upstream 挂点配对）④错误代价足够高 ⑤修改代价足够低 ⑥能机器验证——缺关键一项不机制化。分工：Retro 决定什么值得改变，Maker 把已决定的部分以最低成本变稳定能力，Reactor 保证认知存续',
  ],
}
